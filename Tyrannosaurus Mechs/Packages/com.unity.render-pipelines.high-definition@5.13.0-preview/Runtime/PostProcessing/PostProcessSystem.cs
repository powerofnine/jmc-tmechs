...
namespace UnityEngine.Experimental.Rendering.HDPipeline
{
    ...
    public sealed class PostProcessSystem
    {
        ...
        public PostProcessSystem(HDRenderPipelineAsset hdAsset)
        {
            ...
            m_OutlineMaterial = CoreUtils.CreateEngineMaterial(Shader.Find("Hidden/TMechs/PP/Outline"));
        ...
        public void Render(CommandBuffer cmd, HDCamera camera, BlueNoise blueNoise, RTHandle colorBuffer, RTHandle afterPostProcessTexture, RTHandle lightingBuffer, RenderTargetIdentifier finalRT, RTHandle depthBuffer, bool flipY)
        {
            ...
            using (new ProfilingSample(cmd, "Post-processing", CustomSamplerId.PostProcessing.GetSampler()))
            {
                ...
                if (m_PostProcessEnabled)
                {
                    ...
                    {
                        using (new ProfilingSample(cmd, "Outline", CustomSamplerId.Outline.GetSampler()))
                        {
                            var destination = m_Pool.Get(Vector2.one, k_ColorFormat);
                            DoOutline(cmd, camera, source, destination);
                            PoolSource(ref source, destination);
                        }
                    }
        ...
        void DoColorGrading(CommandBuffer cmd, ComputeShader cs, int kernel)
        {
            ...
            switch (m_Tonemapping.mode.value)
            {
                case TonemappingMode.Neutral:
                    kernelName = "KBuild_NeutralTonemap";
                    break;
                case TonemappingMode.ACES:
                    kernelName = "KBuild_AcesTonemap";
                    break;
                case TonemappingMode.Custom:
                    kernelName = "KBuild_CustomTonemap";
                    break;
                default:
                    kernelName = "KBuild_NoTonemap";
                    break;
            }
        ...
        void DoOutline(CommandBuffer cmd, HDCamera camera, RTHandle source, RTHandle destination /*, RTHandle depthBuffer*/)
        {
            m_OutlineMaterial.SetFloat("_Scale", m_Outline.scale.value);
            m_OutlineMaterial.SetColor("_Color", m_Outline.color.value);
            m_OutlineMaterial.SetFloat("_DepthThreshold", m_Outline.depthThreshold.value);
            m_OutlineMaterial.SetFloat("_DepthNormalThreshold", m_Outline.depthNormalThreshold.value);
            m_OutlineMaterial.SetFloat("_DepthNormalThresholdScale", m_Outline.depthNormalThresholdScale.value);
            m_OutlineMaterial.SetFloat("_NormalThreshold", m_Outline.normalThreshold.value);
            m_OutlineMaterial.SetColor("_Color", m_Outline.color.value);

            Matrix4x4 clipToView = GL.GetGPUProjectionMatrix(camera.projMatrix, true).inverse;
            m_OutlineMaterial.SetMatrix("_ClipToView", clipToView);

            cmd.BlitFullscreenTriangle(source, destination, m_OutlineMaterial, 0);
        }
    ...
    public static class CustomUtils
    {
        ...
        public static Material copyMaterial
        {
            get
            {
                ...
                s_CopyMaterial = new Material(Shader.Find("Hidden/PostProcessing/Copy"))
                {
        ...
        {
            cmd.SetGlobalTexture(HDShaderIDs._MainTex, source);
        ...
        /// <param name="viewport">An optional viewport to consider for the blit</param>
        public static void BlitFullscreenTriangle(this CommandBuffer cmd, RenderTargetIdentifier source, RenderTargetIdentifier destination, Material propertySheet, int pass, RenderBufferLoadAction loadAction, Rect? viewport = null)
        {
            cmd.SetGlobalTexture(HDShaderIDs._MainTex, source);
            ...
            cmd.DrawMesh(fullscreenTriangle, Matrix4x4.identity, propertySheet, 0, pass);
        }
        ...
        /// <param name="clear">Should the destination target be cleared?</param>
        /// <param name="viewport">An optional viewport to consider for the blit</param>
        public static void BlitFullscreenTriangle(this CommandBuffer cmd, RenderTargetIdentifier source, RenderTargetIdentifier destination, Material propertySheet, int pass, bool clear = false, Rect? viewport = null)
        {
