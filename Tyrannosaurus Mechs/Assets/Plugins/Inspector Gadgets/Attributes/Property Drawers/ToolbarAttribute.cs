...
namespace InspectorGadgets.Editor.PropertyDrawers
{
    ...
    internal sealed class ToolbarDrawer : PropertyDrawer
    {
        ...
        private void Initialise(SerializedProperty property, GUIContent label)
        {
            ...
            if (_FieldType == typeof(bool))// Bool.
            {
                ...
                if (_Labels == null || _Labels.Length != 2)
                {
                    ...
                    {
                        Debug.LogWarning(GetImproperAttributeWarningPrefix() + " must specify exactly two labels to be used on a bool field.");
            ...
            else if (_FieldType == typeof(string))// String.
            {
                ...
                if (_Labels == null)
                {
                    Debug.LogWarning(GetImproperAttributeWarningPrefix() + " must specify the labels you with to display.");
                    _Labels = new GUIContent[0];
                    return;
                }
            ...
            else if (_FieldType.IsEnum)// Enum.
            {
                if (_Attribute.Labels != null)
                    Debug.LogWarning(GetImproperAttributeWarningPrefix() + " shound not specify any labels since it is an enum field.");
            ...
            {
                _Labels = new GUIContent[0];
                Debug.LogWarning(GetImproperAttributeWarningPrefix() + " will do nothing because it is only valid on bool, string, and enum fields.");
                return;
            }
        ...
        {
            return "The [Toolbar] attribute applied to " + fieldInfo.DeclaringType.FullName + "." + fieldInfo.Name;
        }
